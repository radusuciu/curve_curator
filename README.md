...
For SPECTRONAUT, export either a normal (long) report or a run pivot report. A normal report takes its experiment names from R.Condition, so name your conditions in the condition setup to match the experiments array in the toml file. A run pivot report instead heads one quantity column per run with that run's label, so there it is the run label that has to match; setting the run label to the condition name makes both report formats produce the same experiment names. The protein-based analysis expects PG.Quantity, the peptide-based analysis expects PEP.Quantity and PEP.GroupingKey. Both need PG.ProteinGroups, and PG.Genes is used for the gene annotation if the report contains it. Reports are read from .tsv and from .parquet files. If the report contains EG.IsDecoy, decoys are removed for you, so there is no need to filter the report first.
