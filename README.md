...
For SPECTRONAUT, label your runs in the condition setup so that the run labels match the experiments array in the toml file. Spectronaut quantifies per run, so one run is one experiment, and replicates are separate experiments as they are for every other search engine. Export either a normal (long) report or a run pivot report: a normal report needs the R.Label column, a run pivot report carries the same run label in its quantity headers instead, and both name their experiments identically, so either export works without further adjustment. The protein-based analysis expects PG.Quantity, the peptide-based analysis expects PEP.Quantity and PEP.GroupingKey. Both need PG.ProteinGroups, and PG.Genes is used for the gene annotation if the report contains it. Reports are read from .tsv and from .parquet files. If the report contains EG.IsDecoy, decoys are removed for you, so there is no need to filter the report first.
